...
Record({
    ...
    data: {
        ...
        sys_ui_section: example_section, // Record<'sys_ui_section'> that references either a new or existing sys_ui_section record
        sys_ui_formatter: get_sys_id('sys_ui_formatter', '') // optional, sys_id | Record<'sys_ui_formatter'>
    }
})
...
const example_form_section = Record({
    ...
    data: {
        sys_ui_form: example_form, // string, the form to display the section on
        sys_ui_section: example_section, // string, the section to display
