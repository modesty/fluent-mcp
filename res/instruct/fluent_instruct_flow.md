...
8. MVP limitations (SDK 4.3.0): subflow invocation, try/catch error handling, step rewind (Go-Back-To), and DateTime flow variables are not yet supported. Use `StringColumn` as a workaround for DateTime flow variables.
...
10. For `runAs: 'system'`, the flow runs under the system context (no user-specific restrictions). Use `runAs: 'user'` when the flow should respect the triggering user's permissions.
