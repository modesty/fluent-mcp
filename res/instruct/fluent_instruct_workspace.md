...
11. UxList supports extensive visibility toggle properties (all boolean, default false): `hideColumnGrouping`, `hidePersonalization`, `hideQuickEdit`, `hideLinks`, `hideMenuButton`, `hidePagination` (split into `hideFirstPage`, `hideLastPage`, `hideNextPage`, `hidePreviousPage`, `hidePages`, `hideRowsPerPageSelector`), `hideRecordCountBadge`, `hideTitle`, etc. Use `groups` array to restrict list access to specific user groups. Use `liveUpdates: true` to enable real-time list updates.
