...
write_report() {
  mkdir -p "$(dirname "$REPORT_FILE")"
  {
    ...
    echo ""
    echo "Generated by \`scripts/validate-snippets.sh\` against \`@servicenow/sdk@4.10.1\` on $(date -u +"%Y-%m-%dT%H:%M:%SZ")."
    echo ""
    echo "## Summary"
    echo ""
    echo "| Status | Count |"
...
main() {
  echo ""
  printf "${BOLD}Validating snippets against now-sdk v4.10.1 build...${NC}\n"
  echo ""
