 ...
 *   - Symlink node_modules to the project root (contains @servicenow/sdk@4.6.0)
...
function setupScaffold() {
  ...
  // Symlink node_modules to project root (where @servicenow/sdk@4.6.0 lives)
  fs.symlinkSync(
  ...
  const fluentGlobals = [
    ...
    'ClientScript', 'CrossScopePrivilege', 'Documentation', 'ImportSet',
...
function writeReport(specs, errorsByFile) {
  ...
  lines.push('');
  lines.push(`Generated by \`scripts/validate-specs.mjs\` against \`@servicenow/sdk@4.6.0\` types on ${new Date().toISOString()}.`);
  lines.push('');
  lines.push('## Summary');
  lines.push('');
...
function main() {
  console.log(`\nValidating res/spec/*.md against @servicenow/sdk@4.6.0 types…\n`);
